...
def load_and_prepare_hepmass():
    ...
    train_set = pd.read_csv('hepmass_train_1000.csv')
    valid_set = pd.read_csv('hepmass_valid_200.csv')
    test_set = pd.read_csv('hepmass_test_500.csv')
    ...
    if method_name == "DataOob":
        ...
        evaluators = [
            DataShapley(
                mc_epochs=mc_epochs, 
                min_cardinality=5,
                cache_name=f"shapley_mc{mc_epochs}_run{run_idx}_SEED_{SEED}_HEPMASS_56",
                random_state=run_idx
            )
            for mc_epochs in [20000]
...
def run_method_experiment(method_name):
    ...
    base_remote_dir = (
        f"/home/mehdi.touil/lustre/scalableml-um6p-st-sccs-10v5rwpbsmu/touil-lustre/Hepmass1K"
    )
