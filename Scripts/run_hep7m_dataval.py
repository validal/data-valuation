...
parser.add_argument('--inrun_batch_size', type=int, default=None,
                   help='InRunDataShapleyGhost: batch_size (default: 1024)')
parser.add_argument('--inrun_epochs', type=int, default=None,
                   help='InRunDataShapleyGhost: epochs (default: 5)')
parser.add_argument('--inrun_lr', type=float, default=None,
                   help='InRunDataShapleyGhost: learning_rate (default: 0.001)')
parser.add_argument('--dvrl_epochs_list', type=str, default=None,
                   help='DVRL: comma-separated rl_epochs values (default: 1000,2000,3000)')
parser.add_argument('--dvrl_batch_size', type=int, default=None,
                   help='DVRL: rl_batch_size (default: sweeps 32,64,128,256,512)')
parser.add_argument('--dvrl_extraction_batch_size', type=int, default=None,
                   help='DVRL: extraction_batch_size for the post-training final-weights '
                        'pass (independent of rl_batch_size; default: falls back to rl_batch_size)')
...
DVRL_EXTRACTION_BATCH_SIZE = args.dvrl_extraction_batch_size
LOGRA_BATCH_SIZE = args.logra_batch_size
INRUN_BATCH_SIZE = args.inrun_batch_size
INRUN_EPOCHS = args.inrun_epochs
INRUN_LR = args.inrun_lr
...
def load_and_prepare_hepmass():
    ...
    data_dir = "/home/mehdi.touil/lustre/scalableml-um6p-st-sccs-10v5rwpbsmu/touil-lustre/Fine_grained_valuation/Revision/Hep/Data"

    # Load datasets from CSV files
    train_set = pd.read_csv(os.path.join(data_dir, 'hepmass_train_7M.csv')).drop(columns=['Unnamed: 0'])
...
def create_method_evaluators(method_name, output_dir=None):
    ...
    if method_name == "DataOob":
        PROPORTIONS = [0.1] #[1.0, 0.7, 0.5, 0.1]
        MODEL_SIZES = [10] #[1,10, 50, 100]
        evaluators = [
        ...
        #                     )
        #                 )
        evaluators = [KNNShapleyLSH(k_neighbors=1000, dist_rand=7.3622, n_hash_table=100, eps=0.0001, alpha=0.5, t=2.399, random_state=SEED)]
    
    elif method_name == "DVRL":
        BATCH_SIZES = [DVRL_BATCH_SIZE] if DVRL_BATCH_SIZE is not None else [32,64,128,256,512]
        MODEL_SIZES = DVRL_EPOCHS_LIST if DVRL_EPOCHS_LIST is not None else [1000,2000,3000]
        evaluators = []
        ...
        EPOCHS = INRUN_EPOCHS if INRUN_EPOCHS is not None else 5
        BATCH_SIZE = INRUN_BATCH_SIZE if INRUN_BATCH_SIZE is not None else 1024
        LR = INRUN_LR if INRUN_LR is not None else 0.001
        ...
        evaluators = [
            InRunDataShapleyGhost(
                ...
                plot_dir=plots_dir,  # ← Save plots in results folder
                # OneCycleLR (the default) divides by zero on a ragged last
                # batch -- likely with batch_size=100000/500000 against a
                # train set that won't divide evenly. scheduler_type="none"
                # disables the scheduler, optimizer uses a fixed learning_rate.
                scheduler_type="none"
            )
            for s in [SEED]
        ]
        ...
        EPOCHS = 5
        BATCH_SIZE = LOGRA_BATCH_SIZE if LOGRA_BATCH_SIZE is not None else 1024
        LR = 0.001
        ...
        LAMDA = [0,0.8,0.9,0.97,0.98,0.99,1.0]
        #LAMDA = [0.97]
...
def run_method_experiment(method_name):
    ...
    base_remote_dir = (
        f"/home/mehdi.touil/lustre/scalableml-um6p-st-sccs-10v5rwpbsmu/touil-lustre/Fine_grained_valuation/Revision/Hep/results"
    )
...
def run_all_methods():
    ...
    methods = ['DataOob', 'AME', 'DataBanzhaf', 'DataShapley',
               'InfluenceSubsample', 'LOO_Random', 'KNNShapley','AKShapley'
               'DVRL', 'BetaShapley', 'LAVA', 'InRunDataShapleyGhost',
               'LoGRA', 'Kairos']

    # Create scripts directory
    scripts_dir = "/home/mehdi.touil/lustre/scalableml-um6p-st-sccs-10v5rwpbsmu/touil-lustre/Fine_grained_valuation/Revision/Hep/scripts"
    logs_dir = "/home/mehdi.touil/lustre/scalableml-um6p-st-sccs-10v5rwpbsmu/touil-lustre/Fine_grained_valuation/Revision/Hep/logs"
...
#SBATCH --output=/home/mehdi.touil/lustre/scalableml-um6p-st-sccs-10v5rwpbsmu/touil-lustre/Fine_grained_valuation/Revision/Hep/logs/run_hepmass_{method}_%j.log
#SBATCH --error=/home/mehdi.touil/lustre/scalableml-um6p-st-sccs-10v5rwpbsmu/touil-lustre/Fine_grained_valuation/Revision/Hep/logs/run_hepmass_{method}_%j.err
...
# Activate Python 3.12 virtual environment (GhostSuite)
...
python /home/mehdi.touil/lustre/scalableml-um6p-st-sccs-10v5rwpbsmu/touil-lustre/Fine_grained_valuation/Revision/Hep/run_hepmass_dataval_7M.py --seed 42 --method {method} --job_id $SLURM_JOB_ID
...
"""
    ...
    print(f"Results will be saved to: /home/mehdi.touil/lustre/scalableml-um6p-st-sccs-10v5rwpbsmu/touil-lustre/Fine_grained_valuation/Revision/Hep/results/SEED{SEED}")
