...
def load_and_prepare_hepmass():
    ...
    train_set = pd.read_csv('hepmass_train_1M.csv')
    valid_set = pd.read_csv('hepmass_valid_0.2M.csv')  # Load only 200k rows for validation
    test_set = pd.read_csv('hepmass_test_0.5M.csv')
    ...
    shuffle_idx = np.random.default_rng(SEED).permutation(len(X_train))
    X_train = X_train.iloc[shuffle_idx]  # Changed from X_train[shuffle_idx]
    Y_train = Y_train.iloc[shuffle_idx]  # Changed from y_train[shuffle_idx]
...
def create_method_evaluators(method_name,exper_med):
    ...
    #     evaluators = [KNNShapley(k_neighbors=k,debug=True,batch_size=512) for k in k_values]
        ...
        #evaluator = KNNShapleyLSH(k_neighbors=10,dist_rand = 7 , n_hash_table=500, eps=0.01,alpha=0.1,t=2)
        #evaluator = KNNShapleyLSH(k_neighbors=10,dist_rand = 7 , n_hash_table=500, eps=0.01,alpha=0.1,t=2)
        #evaluator = KNNShapleyLSH(k_neighbors=100,dist_rand = 7.3622 , n_hash_table=100, eps=0.0002,alpha=0.5,t=2.3990)
        #evaluator = KNNShapleyLSH(k_neighbors=1000, dist_rand=7.3622, n_hash_table=100, eps=0.0002, alpha=0.5, t=2.3990)
        #evaluator =  KNNShapleyLSH(k_neighbors=100, dist_rand=7.3622, n_hash_table=200, eps=0.0001, alpha=0.5, t=2.399)
        evaluator = KNNShapleyLSH(k_neighbors=100, dist_rand=7.3622, n_hash_table=100, eps=0.00005, alpha=0.5, t=2.399,random_state=10)
        ...
        dist_cache_file = f"hepmass_dist_matrix_SEED{SEED}_1M.pt"
        ...
        # rl_epochs = [2000,5000,10000]
        evaluators = []
        # for rl_epochs in rl_epochs:
        ...
        evaluators = [
            SavaEvaluator( batch_size=SAVA_BATCH_SIZE,
            lam_x=1.0,
            lam_y=10.0,
            debug=True,
            random_state=RS,
...
def run_method_experiment(method_name):
    ...
    BASE_OUTPUT_DIR = (
    "/home/mehdi.touil/lustre/"
    "scalableml-um6p-st-sccs-10v5rwpbsmu/"
    "touil-lustre/HepMass1M/"
)
