...
parser.add_argument(
    ...
    type=int,
    default=None,
    help="Subset size for InfluenceSubsample. If not set, use default (16)."
)
parser.add_argument(
    "--num_models",
    type=int,
    default=None,
    help="Number of models for InfluenceSubsample. If not set, use default (100000)."
)
parser.add_argument(
    "--oob_num_models",
    type=int,
    default=None,
    help="Number of models for DataOob. If not set, use default (10)."
)
parser.add_argument(
    "--oob_proportion",
    type=float,
    default=None,
    help="Bootstrap proportion for DataOob. If not set, use default (1.0)."
)
...
METHOD = args.method
LAM_Y = args.lam_y
SUBSET_SIZE = args.subset_size if args.subset_size is not None else 16
NUM_MODELS = args.num_models if args.num_models is not None else 100000
OOB_NUM_MODELS = args.oob_num_models
OOB_PROPORTION = args.oob_proportion
...
def load_and_prepare_hepmass():
    """Load HEPMASS 10K datasets and prepare them for the experiment."""
    print("Loading HEPMASS 10K datasets...")

    # Use absolute paths for data files
    data_dir = "/srv/lustre01/project/scalableml-um6p-st-sccs-10v5rwpbsmu/touil-lustre/Fine_grained_valuation/Revision/Hep10K/Data"

    # Load datasets from CSV files
    train_set = pd.read_csv(os.path.join(data_dir, 'hepmass_train_10000.csv'))
    valid_set = pd.read_csv(os.path.join(data_dir, 'hepmass_valid_2000.csv'))
    test_set = pd.read_csv(os.path.join(data_dir, 'hepmass_test_5000.csv'))
...
def create_experiment_mediator():
    """Create and configure the ExperimentMediator for HEPMASS 10K.

    CRITICAL: Global seeds are set at the beginning to ensure model
    weight initialization is reproducible.
    """
    print("Creating HEPMASS 10K experiment mediator...")
...
def create_method_evaluators(method_name, output_dir=None):
    ...
    if method_name == "DataOob":
        PROPORTIONS = [OOB_PROPORTION] if OOB_PROPORTION is not None else [1.0]
        MODEL_SIZES = [OOB_NUM_MODELS] if OOB_NUM_MODELS is not None else [10]
        evaluators = [
        ...
        for m in [NUM_MODELS]:  # Use command-line argument or default
            for s in [SEED]:  # 1..10
                evaluators.append(
                    InfluenceSubsample(
                        num_models=m,
                        ...
                        # rng=SEED (confirmed leak: RandomState(seed)
        ...
        evaluators = [
            DataShapley(mc_epochs=m, min_cardinality=5, cache_name="shapley_cache", random_state=s)
            for m in [100]
            for s in [SEED]
        ]
    elif method_name == "LAVA":
        LAM_Y = [1,2,5,10,15,20,25,30,40,50,60,70,80,100]
        evaluators = [
        ...
        evaluators = [
            InRunDataShapleyGhost(
                ...
                # is small (n_train=10000, batch_size=1024 -> only 9 batches/epoch
                # x 5 = 45 steps). lr_peak_epoch=1 keeps pct_start=0.2, normal.
                lr_peak_epoch=1,
            )
            for s in [SEED]
...
def run_method_experiment(method_name):
    ...
    base_remote_dir = (
        f"/srv/lustre01/project/scalableml-um6p-st-sccs-10v5rwpbsmu/touil-lustre/Fine_grained_valuation/Revision/Hep10K/results"
    )
    ...
    with open(summary_path, 'w') as f:
        f.write(f"HEPMASS 10K Data Valuation - {method_name}\n")
        f.write("=" * 50 + "\n")
