...
parser.add_argument('--seed', type=int, default=42, help='Random seed for the setup')
parser.add_argument('--rs', type=int, default= 1, help='Random seed for the valuation')
...
parser.add_argument('--batch_size', type=int, default=512, help='Batch size for SAVA evaluator')
# KNNShapleyLSH parameters
parser.add_argument('--k_neighbors', type=int, default=10, help='k parameter for KNNShapleyLSH')
parser.add_argument('--n_hash_table', type=int, default=100, help='n_hash_table parameter for KNNShapleyLSH')
parser.add_argument('--eps', type=float, default=0.01, help='eps parameter for KNNShapleyLSH')
parser.add_argument('--alpha', type=float, default=0.5, help='alpha parameter for KNNShapleyLSH')
...
print(f"  - RANDOM SEED (valuation): {RS}")
print(f"  - METHOD: {METHOD}")
print(f"  - JOB_ID: {JOB_ID}")
print(f"  - SAVA_BATCH_SIZE: {SAVA_BATCH_SIZE}")
...
def load_and_prepare_hepmass():
    ...
    train_set = pd.read_csv('hepmass_train_100K.csv')
    valid_set = pd.read_csv('hepmass_valid_20K.csv')
    test_set = pd.read_csv('hepmass_test_50K.csv')
...
def create_method_evaluators(method_name,exper_med):
    ...
    #     evaluators = [KNNShapley(k_neighbors=k,debug=True,batch_size=512) for k in k_values]
        ...
        evaluators = [LavaEvaluator(lam_y=10, debug=True, blur=0.05)]

    elif method_name == "SAVA":
        evaluators = [SavaEvaluator(batch_size=SAVA_BATCH_SIZE, lam_x=1, lam_y=10, debug=True,random_state=rs,stratified_batches=True)]
    #     evaluators = [SavaEvaluator(
    #     batch_size=1024,        # Safe size (4096² = 16.7M distances)
    #     lam_x=1.0,              # Feature weight
    #     lam_y=1.0,              # Label weight (reduced from 10)
    #     entreg=0.1,             # Sinkhorn regularization
    #     blur=0.05,              # Entropic scale
    #     scaling=0.8,            # Epsilon scaling
    #     sinkhorn_reg=1e-2,      # Outer regularization
    #     debug=True,
...
def run_method_experiment(method_name):
    ...
    # Create output directory with method name (remote VSCode path)
    if args.output_dir:
        output_dir = args.output_dir
    else:
        base_remote_dir = (
            f"/home/mehdi.touil/lustre/scalableml-um6p-st-sccs-10v5rwpbsmu/touil-lustre/Hepmass100K/"
        )
        output_dir = os.path.join(base_remote_dir, method_name, f'knn_shapley_{JOB_ID}')
...
METHODS=("DataOob" "AME" "DataBanzhaf" "DataShapley" "InfluenceSubsample" 
         "LOO_Random" "KNNShapley" "DVRL" "BetaShapley" "LAVA" "SAVA")
...
# ---------------------------------
cd "/home/mehdi.touil/ondemand/Experimental evaluation/Hepmass/HepMASS100K"
...
python run_hepmass_dataval_100k.py --seed 42 --method {method} --job_id $SLURM_JOB_ID
