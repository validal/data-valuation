...
def load_full_dataset(data, targets=False, return_both_targets=False,
    ...
    for batch in tqdm(loader, leave=False):
        ...
        if collect_targets: # = True or infer
            # Ensure target tensor is 1-D per batch to avoid zero-dim entries
            y_t = y.to(device)
            if y_t.dim() == 0:
                # single scalar -> make length-1 vector
                y_t = y_t.unsqueeze(0)
            else:
                # collapse trailing dimensions into feature dim, keep batch dim
                y_t = y_t.reshape(y_t.shape[0], -1)
                # if single-column, squeeze to 1-D vector
                if y_t.shape[1] == 1:
                    y_t = y_t[:, 0]
            Y.append(y_t)
    X = torch.cat(X)
