...
#         if not self.debiased_loss:
    ...
    def distance(self, maxsamples=10000, return_coupling=False):
        ...
        if (self.n1 > GPU_LIMIT or self.n2 > GPU_LIMIT) and maxsamples > GPU_LIMIT and self.device != 'cpu':
            logger.warning('Warning: maxsamples = {} > 50000, and device = {}. Loaded data' \
...
#         first_dist = []
    ...
    def dual_sol(self, maxsamples=10000, return_coupling=False):
        ...
        GPU_LIMIT = 100000
        if (self.n1 > GPU_LIMIT or self.n2 > GPU_LIMIT) and maxsamples > GPU_LIMIT and self.device != 'cpu':
            logger.warning('Warning: maxsamples = {} > 40000, and device = {}. Loaded data' \
                   ' might not fit in GPU. Computing distances on' \
                   ' CPU.'.format(maxsamples, self.device))
            device_dists = 'cpu'
        if self.X1 is None or self.X2 is None:
...
class FeatureCost():
    ...
    def __call__(self, X1, X2):
        ...
        try:
            print(f"[FeatureCost] embedded shapes: X1={tuple(X1.shape)}, X2={tuple(X2.shape)}")
        except Exception:
            pass
