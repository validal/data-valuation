...
class OTDD_Gradient_Flow(GradientFlow):
    ...
    def label_update(self):
        ...
        if cmethod == 'kmeans':
            ...
            # Enforce a minimum cluster size to avoid singleton clusters
            # If a cluster has fewer than `min_size` members, reassign its
            # points to the nearest other centroid. This stabilizes clustering
            # for downstream computations that assume non-empty clusters.
            min_size = 2
            # ensure L is a numpy array of ints
            L = np.asarray(L, dtype=int)
            counts = np.bincount(L, minlength=k)
            # Repeat a few times to stabilize reassignment when multiple tiny
            # clusters exist. Bound iterations to k to avoid infinite loops.
            for _it in range(k):
                ...
                for s in small:
                    ...
                    for i in idxs:
                        ...
                        dists = np.linalg.norm(U[i] - C, axis=1)
                        # prevent reassignment to same (tiny) cluster
                        dists[s] = np.inf
