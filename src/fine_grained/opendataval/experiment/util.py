...
class CheckpointManager:
    """Manages model checkpoints for efficient data valuation."""
    ...
    def __init__(self, checkpoint_interval: int = 5):
        ...
        checkpoint_interval : int
            Save checkpoint at every N epochs
        """
    ...
    def save_checkpoint(self, epoch: int, model):
        """Save model state at given epoch."""
        # If already a dict (state_dict), use it directly
        if isinstance(model, dict):
            self.checkpoints[epoch] = model.copy()
        # If model has state_dict method, call it
        elif hasattr(model, 'state_dict'):
            self.checkpoints[epoch] = model.state_dict().copy()
        # Otherwise extract __dict__
        else:
            self.checkpoints[epoch] = {k: v.clone() if isinstance(v, torch.Tensor) else v
    ...
    def get_checkpoints_for_epochs(self, epoch_list: list) -> dict:
        """Get checkpoints for specific epochs."""
        return {epoch: self.checkpoints[epoch]
                for epoch in epoch_list if epoch in self.checkpoints}
