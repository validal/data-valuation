...
class TorchClassMixin(TorchModel):
    ...
    def fit(
        ...
        self.train()
        pbar = tqdm(total=int(epochs), desc="Training", unit="epoch", position=0, leave=True)
        for epoch in range(int(epochs)):
            ...
            for x_batch, y_batch, *weights in DataLoader(
                ...
                # Prepare targets based on criterion expectations
                if self.num_classes > 2:
                    # Cross-entropy expects class indices LongTensor of shape [N]
                    if y_batch.dim() > 1:
                    ...
                    if sample_weight is not None:
                        # F.cross_entropy doesn't support sample_weights directly
                    ...
                    # Binary case uses BCE on probabilities/logits-mapped probs
                    # Ensure target is float and shaped like outputs
                    y_t = y_batch.to(dtype=outputs.dtype, device=self.device)
                    if y_t.dim() == 1 or (y_t.dim() == 2 and y_t.shape[1] == 1 and outputs.dim() == 2 and outputs.shape[1] == 2):
                        # If outputs are 2-dim classes with BCE, convert label to one-hot
                        num_c = outputs.shape[1]
                        y_idx = y_t.view(-1).to(dtype=torch.long)
                        y_t = torch.zeros((y_idx.shape[0], num_c), device=self.device, dtype=outputs.dtype)
                        y_t.scatter_(1, y_idx.unsqueeze(1), 1.0)
                    if sample_weight is not None:
                        loss = criterion(outputs, y_t, reduction="none")
                        loss = (loss * weights[0].to(device=self.device)).mean()
                    else:
...
def to_numpy(tensors: tuple[torch.Tensor]) -> tuple[torch.Tensor]:
    ...
    #return tuple(t.detach().cpu().numpy() for t in default_collate(tensors))
    return tuple(t.detach().cpu().numpy(force=True) if hasattr(t, "numpy") else t for t in default_collate(tensors))
...
class ClassifierSkLearnWrapper(Model):
    ...
    def fit(
        ...
        **kwargs,
        ...
        # Support integer labels or one-hot labels
        if hasattr(y_train, "ndim") and (y_train.ndim == 1 or (y_train.ndim == 2 and y_train.shape[1] == 1)):
