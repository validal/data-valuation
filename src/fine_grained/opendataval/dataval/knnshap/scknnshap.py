...
class LSH:
    def __init__(self, n_hash_bit, n_hash_table, x_trn, y_trn, dist_rand, equal, t=0.1):
        ...
        for i in progress_range(self.N, "Building LSH index"):
    ...
    def get_approx_KNN(self, x_tst, K):
        ...
        for i_tst in progress_range(N_tst, "Querying LSH"):
            nns = []
    ...
    def compute_approx_shapley(self, x_tst_knn, y_tst, K):
        ...
        for j in progress_range(N_tst, "Computing Shapley values"):
...
class KNNShapleyLSH(DataEvaluator, ModelLessMixin):
    ...
    embedding_model : Model, optional
        Pre-trained embedding model for computing embeddings, by default None.
    ...
    def input_data(self, x_train, y_train, x_valid, y_valid):
        # Store original data (may be torch.Tensor or Dataset) for embedding
    ...
    def compute_lsh_recall(self, K: Optional[int] = None, n_sample: int = 200, random_state: Optional[int] = None):
        r"""Empirical Recall\@K on a random sample of validation points.
