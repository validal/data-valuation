...
class AME(DataEvaluator, ModelMixin):
    ...
    def evaluate_data_values(self) -> np.ndarray:
        ...
        df_results = pd.DataFrame(self.subsets)
        

        # Add the target (y) and the calculated AME scores
        #df_results['y_centered_perf'] = self.performance

        # 4. Save to CSV
        #df_results.to_csv('ame_valuation_results.csv', index=False)

        #print("Saved ame_valuation_results.csv successfully.")


        dv_ame = LassoCV(random_state=self.random_state)
        #dv_ame = ElasticNetCV(random_state=self.random_state, l1_ratio=0.7)
