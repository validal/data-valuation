import numpy as np
import torch
from opendataval.dataval.progress import ProgressBar, progress_range
from torch.utils.data import Subset

from opendataval.dataval.api import DataEvaluator, ModelMixin


class LeaveOneOut(DataEvaluator, ModelMixin):
    """Leave One Out data valuation implementation.

    References
    ----------
    .. [1] R. Cook,
        Detection of Influential Observation in Linear Regression,
        Technometrics, Vol. 19, No. 1 (Feb., 1977), pp. 15-18 (4 pages).

    Parameters
    ----------
    random_state : RandomState, optional
        Random initial state, by default None
    """

    def __init__(self, random_state=None):
        """Initialize LeaveOneOut with random state.

        Parameters
        ----------
        random_state : RandomState, optional
            Random initial state for reproducibility, by default None
        """
        super().__init__(random_state=random_state)

    def input_data(
        self,
        x_train: torch.Tensor,
        y_train: torch.Tensor,
        x_valid: torch.Tensor,
        y_valid: torch.Tensor,
    ):
        """Store and transform input data for Leave-One-Out data valuation.

        Parameters
        ----------
        x_train : torch.Tensor | Dataset
            Data covariates
        y_train : torch.Tensor
            Data labels
        x_valid : torch.Tensor | Dataset
            Test+Held-out covariates
        y_valid : torch.Tensor
            Test+Held-out labels
        """
        self.x_train = x_train
        self.y_train = y_train
        self.x_valid = x_valid
        self.y_valid = y_valid

        # Additional parameters
        self.num_points = len(x_train)

        return self

    def train_data_values(self, *args, **kwargs):
        """Trains model to predict data values.

        Compute the data values using the Leave-One-Out data valuation.
        Equivalently, LOO can be computed from the marginal contributions as it's a
        semivalue.

        Parameters
        ----------
        args : tuple[Any], optional
            Training positional args
        kwargs : dict[str, Any], optional
            Training key word arguments
        """
        self.data_values = np.zeros((self.num_points,))
        indices = self.random_state.permutation(self.num_points)

        curr_model = self.pred_model.clone()

        curr_model.fit(self.x_train, self.y_train, *args, **kwargs)
        y_valid_hat = curr_model.predict(self.x_valid)
        baseline_score = self.evaluate(self.y_valid, y_valid_hat)

        for i in progress_range(self.num_points):
            loo_coalition = np.delete(indices, i)  # Deletes random point

            curr_model = self.pred_model.clone()
            curr_model.fit(
                Subset(self.x_train, indices=loo_coalition),
                Subset(self.y_train, indices=loo_coalition),
                *args,
                **kwargs,
            )

            y_hat = curr_model.predict(self.x_valid)
            loo_score = self.evaluate(self.y_valid, y_hat)
            #print(f"LOO score for point {i}: {baseline_score - loo_score:.20f}")
            self.data_values[indices[i]] = baseline_score - loo_score

        return self

    def evaluate_data_values(self) -> np.ndarray:
        """Compute data values using Leave One Out data valuation.

        Returns
        -------
        np.ndarray
            Predicted data values/selection for training input data point
        """
        return self.data_values
    def train_group_data_values(self, groups: list[list[int]], *args, **kwargs):
        """
        Trains model to predict data values for groups of points.

        Parameters
        ----------
        groups : list of list of int
            Each sublist contains indices of points to be considered as a group.
        """
        self.group_data_values = np.zeros((len(groups),))
        indices = np.arange(self.num_points)

        curr_model = self.pred_model.clone()
        curr_model.fit(self.x_train, self.y_train, *args, **kwargs)
        y_valid_hat = curr_model.predict(self.x_valid)
        baseline_score = self.evaluate(self.y_valid, y_valid_hat)

        for idx, group in enumerate(groups):
            mask = np.ones(self.num_points, dtype=bool)
            mask[group] = False
            group_coalition = indices[mask]

            curr_model = self.pred_model.clone()
            curr_model.fit(
                Subset(self.x_train, indices=group_coalition),
                Subset(self.y_train, indices=group_coalition),
                *args,
                **kwargs,
            )

            y_hat = curr_model.predict(self.x_valid)
            group_score = self.evaluate(self.y_valid, y_hat)
            self.group_data_values[idx] = baseline_score - group_score

        return self

    def evaluate_group_data_values(self) -> np.ndarray:
        """
        Returns the computed group data values.

        Returns
        -------
        np.ndarray
            Data values for each group.
        """
        return self.group_data_values
