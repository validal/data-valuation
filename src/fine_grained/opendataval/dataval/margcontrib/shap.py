...
from opendataval.dataval.margcontrib.sampler import GrTMCSampler, Sampler,MonteCarloSampler,TMCSampler,GroupAwareTMCSampler,TMCSamplerBYnum_models
...
class ShapEvaluator(DataEvaluator, ModelMixin, ABC):
    ...
    def __init__(self, sampler: Optional[Sampler] = None, *args, **kwargs):
        ...
        if self.sampler is None:
            self.sampler = MonteCarloSampler(*args, **kwargs)
    ...
    def _evaluate_model(self, subset: list[int], *args, **kwargs):
        ...
        """
        #print(f"Start model fit for subset: {subset}")
        ...
        #print(f"Performance: {curr_perf}")
        return curr_perf

    def parallel_evaluate(self, subsets, *args, **kwargs):
        results = Parallel(n_jobs=-1)(delayed(self._evaluate_model)(subset, *args, **kwargs) for subset in subsets)
        return results
