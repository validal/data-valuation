...
class DataOob(DataEvaluator, ModelMixin):
    ...
    def train_data_values(self, *args, **kwargs):
        ...
        for i in progress_range(self.num_models, "DataOob"):
            in_bag = subsets[i]
            #print(f"Training model {i+1}/{self.num_models} with {len(in_bag)} in-bag samples.")

            # out_bag is the indices where the bincount is zero.
            out_bag = (np.bincount(in_bag, minlength=self.num_points) == 0).nonzero()[0]
            #print(f"Out-of-bag samples: {len(out_bag)}")
            if not out_bag.any():
                continue
            ...
            print(f"self.oob_pred shape: {self.oob_pred.shape}")

            # Ensure tensors are on CPU and properly shaped
            if len(y_hat.shape) == 1:
                y_hat = y_hat.unsqueeze(1)  # Add dimension if needed

            # Convert to appropriate dtype
            if self.oob_pred.dtype != y_hat.dtype:
                y_hat = y_hat.to(self.oob_pred.dtype)
    ...
    def evaluate_data_values(self) -> np.ndarray:
        ...
        for i, indices in self.oob_indices.items():
            try:
                ...
                y_train_i_shape = getattr(y_train_i, 'shape', 'N/A')
                y_train_i_value = y_train_i.item() if (hasattr(y_train_i, 'numel') and y_train_i.numel() == 1) else y_train_i
                print(f"[DataOob DEBUG] i={i}, y_train[i]: type={type(y_train_i).__name__}, shape={y_train_i_shape}, value type={type(y_train_i_value)}")

                oob_labels = y_train_i.expand((len(indices), *self.label_dim))
                print(f"[DataOob DEBUG] oob_labels: type={type(oob_labels).__name__}, shape={oob_labels.shape}, device={getattr(oob_labels, 'device', 'N/A')}")

                oob_pred_indices = self.oob_pred[indices]
                print(f"[DataOob DEBUG] oob_pred[indices]: type={type(oob_pred_indices).__name__}, shape={getattr(oob_pred_indices, 'shape', 'N/A')}, device={getattr(oob_pred_indices, 'device', 'N/A')}")

                eval_result = self.evaluate(oob_labels, oob_pred_indices)
                print(f"[DataOob DEBUG] evaluate result: type={type(eval_result).__name__}, shape={getattr(eval_result, 'shape', 'scalar')}, device={getattr(eval_result, 'device', 'N/A')}")

                # Convert tensor to scalar if needed
                if hasattr(eval_result, 'item'):
                    print(f"[DataOob DEBUG] Converting tensor to scalar...")
                    eval_result = eval_result.item()
                    print(f"[DataOob DEBUG] Scalar value: {eval_result}, type={type(eval_result)}")

                self.data_values[i] = eval_result
                print(f"[DataOob DEBUG] Successfully assigned data_values[{i}]")
            except Exception as e:
                print(f"[DataOob ERROR] Failed at i={i}: {type(e).__name__}: {e}")
                import traceback
                traceback.print_exc()
                raise
