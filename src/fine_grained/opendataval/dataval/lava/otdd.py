...
class DatasetDistance:
    ...
    def _augmented_cost_matrix(self) -> torch.Tensor:
        ...
        # Flatten features if needed (e.g., for images)
        X1 = self.x_train
        X2 = self.x_valid
        if X1.dim() > 2:
            X1 = X1.view(len(X1), -1)
        ...
        Z1 = torch.cat((X1, self.y_train.float().unsqueeze(dim=1)), -1)
        Z2 = torch.cat((X2, self.y_valid.float().unsqueeze(dim=1)), -1)
...
def extract_dataset(
    ...
    for x, y in ProgressBar(iterable=loader, desc="Loading data"):
...
def pwdist_exact(
    ...
    # Flatten high-dimensional tensors (e.g., images) to 2D for geomloss
    if X1.dim() > 2:
    ...
    for i, j in ProgressBar(iterable=pairs, desc="Computing label-to-label distance"):
