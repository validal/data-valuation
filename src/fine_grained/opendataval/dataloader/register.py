...
def one_hot_encode(data: np.ndarray) -> np.ndarray:
    ...
    # Handle empty arrays (e.g., presplit validation placeholders)
    if len(data) == 0:
        return np.array([]).reshape(0, 10)  # Return empty one-hot with 10 classes
...
class Register:
    ...
    def from_train_test(
        ...
        """Register a dataset given separate train and test arrays.

        This sets the register to `presplit=True` and returns the provided
        train/test arrays, with validation initially mirroring the train pool.

        DataFetcher can then perform sampling of train/valid from the train pool
        and test from the test pool.

        Parameters
        ----------
        x_train : np.ndarray
            Training covariates pool
        y_train : np.ndarray
            Training labels pool
        x_test : np.ndarray
            Test covariates pool
        y_test : np.ndarray
            Test labels pool
        one_hot : bool, optional
            Whether labels are one-hot encoded, by default False
        """
        self.presplit = True
        self.one_hot = one_hot
        self.label_transform = one_hot_encode if one_hot else self.label_transform
        ...
        def _loader():
            # Validation placeholder equals training pool; Fetcher will sample disjointly
            return x_train, x_train, x_test, y_train, y_train, y_test

        self.covar_label_func = _loader
        self.cacheable = False
        return self
    ...
    def load_data(
        ...
        if hasattr(self, "covar_label_func"):
            ...
            if self.presplit:
                # Presplit returns 6 values: (x_train, x_valid, x_test, y_train, y_valid, y_test)
                covar = result[:3]  # (x_train, x_valid, x_test)
                label = result[3:]  # (y_train, y_valid, y_test)
            else:
                # Regular returns 2 values: (covar, label)
                covar, label = result
        else:
