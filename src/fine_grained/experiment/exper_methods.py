...
def noisy_detection(
    ...
    unvaluable, valuable = oned_twonn_clustering(data_values.flatten())
    f1_kmeans_label = f1_score(unvaluable, noisy_train_indices, len(data_values))

    low_k = bottom_k_percent_indices(data_values.flatten(), len(noisy_train_indices))  # type: ignore
    ...
    size_low, mean_low = cluster_stats(unvaluable, data_values.flatten())
    size_high, mean_high = cluster_stats(valuable, data_values.flatten())


    return {"kmeans_f1": f1_kmeans_label,"size_low": size_low, "mean_low": mean_low, "size_high": size_high, "mean_high": mean_high, "Exact_recall": recall_exact_label_value}
...
def remove_high_low(
    ...
    for bin_index in range(0, num_points, num_period):
        ...
        unvaluable_score = metric(y_test.to("cpu"), y_hat_valid)
...
def increasing_bin_removal(
    ...
    for bin_end in bins_indices:
        ...
        perf.append(metric(y_test, y_hat))
