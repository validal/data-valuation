...
class DVRL(DataEvaluator, ModelMixin):
    ...
    def _evaluate_baseline_models(self, *args, **kwargs):
        ...
        # ##################
        # # Uniform-weight baseline (NO subset, NO DVRL)
        # self.uniform_model = self.pred_model.clone()
        # self.uniform_model.fit(self.x_train, self.y_train, *args, **kwargs)

        # y_valid_hat_uniform = self.uniform_model.predict(self.x_valid)
        # self.uniform_valid_perf = self.evaluate(self.y_valid, y_valid_hat_uniform)
        # ##################

        self.y_pred_diff = torch.abs(self.y_train - y_pred)

        # Debug prints for baseline and setup
        try:
            eval_name = self.evaluate.__name__  # function or bound method
        except Exception:
            eval_name = str(type(self.evaluate))

        
        try:
            diff_min = float(self.y_pred_diff.min().item())
            diff_max = float(self.y_pred_diff.max().item())
            diff_mean = float(self.y_pred_diff.mean().item())
        except Exception:
            pass
    ...
    def train_data_values(self, *args, num_workers: int = 0, **kwargs):
        ...
        for x_batch, y_batch, y_hat_batch in tqdm.tqdm(dataloader):
            ...
            # Debug: selection statistics
            try:
                sp = select_prob.detach().cpu()
                selected = int(sp.sum().item())
                total = int(sp.numel())
                rate = selected / max(total, 1)
            except Exception:
                pass
            ...
            try:
                reward_min = min(reward_min, float(reward_curr))
                reward_max = max(reward_max, float(reward_curr))
            except Exception:
                pass
        ...
        try:
            ...
            w_min = float(wcpu.min().item())
            w_max = float(wcpu.max().item())
            w_mean = float(wcpu.mean().item())
            near0 = int((wcpu < 0.1).sum().item())
            near1 = int((wcpu > 0.9).sum().item())
            total_w = int(wcpu.numel())
        except Exception:
            pass
        ...
        # Debug: final model performance against baseline
        try:
            y_valid_hat_final = self.final_model.predict(self.x_valid)
            final_valid_perf = self.evaluate(self.y_valid, y_valid_hat_final)
        except Exception:
            pass
    ...
    def evaluate_data_values(self) -> np.ndarray:
        ...
        # Debug: abs diff stats for evaluation path
        try:
            yhcpu = y_hat.detach().cpu()
            yh_min = float(yhcpu.min().item())
            yh_max = float(yhcpu.max().item())
            yh_mean = float(yhcpu.mean().item())
        except Exception:
            pass
        ...
        # Debug: final data values stats
        try:
            resp_cpu = response.detach().cpu()
            dv_min = float(resp_cpu.min().item())
            dv_max = float(resp_cpu.max().item())
            dv_mean = float(resp_cpu.mean().item())
        except Exception:
            pass
