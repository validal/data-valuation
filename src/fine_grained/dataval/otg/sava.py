...
class BatchwiseLavaEvaluator(DataEvaluator, ModelLessMixin):
    """Batch-wise LAVA data valuation.

    Processes training and validation data in batches. For each
    (train_batch, val_batch) pair, solves a small OT problem and
    accumulates calibrated dual potentials as data values.

    This is the batch-wise extension of LAVA that scales to larger datasets.
    For the full SAVA algorithm (hierarchical OT), use SavaEvaluator.
    ...
    blur : float
        Entropic scale for GeomLoss (epsilon ≈ blur**p).
    scaling : float
        GeomLoss epsilon-scaling factor.
    cache_label_distances : bool
        If True, reuse the label distance matrix W computed from the
        first batch pair across all subsequent pairs (faster but
        approximate when class distributions vary across batches).
    mode : str
    ...
    def _iter_batches(self, x: torch.Tensor, y: torch.Tensor):
        """Yield (x_batch, y_batch) chunks of size self.batch_size."""
        n = len(x)
    ...
    def train_data_values(self, *args, **kwargs):
        """Compute SAVA data values via batch-wise OT.

        For each train batch i and val batch j:
        1. Solve OT → dual potentials (F_i, G_j)
        2. Calibrate F_i → per-point scores
        3. Squash with tanh and normalize
        4. Accumulate across val batches

        Final value per point = mean over val batches.
        """
        x_train, x_valid = self.embeddings(self.x_train, self.x_valid)
        feature_cost = self._make_feature_cost()

        n_train = len(x_train)
        values = np.zeros(n_train, dtype=np.float64)
        counts = np.zeros(n_train, dtype=np.int32)
        ...
        tr_start = 0
        for x_tr, y_tr in tqdm(
            self._iter_batches(x_train_iter, y_train_iter),
            desc="SAVA valuation (train batches)",
            total=int(np.ceil(n_train / self.batch_size)),
        ):
            batch_tr_size = len(x_tr)
            batch_values = np.zeros(batch_tr_size, dtype=np.float64)
            ...
            for x_val, y_val in val_batches:
                ...
                # Cache label distances after first computation
                if self.cache_label_distances and label_distances is None:
                    label_distances = label_distances_new

                assert dual_sol[0].shape[-1] == batch_tr_size, (
                    f"Expected dual_sol[0] size {batch_tr_size}, got {dual_sol[0].shape}"
                )

                calibrated = _calibrate_gradients(dual_sol[0])   # (batch_tr_size,)
                squashed   = _squash_and_normalize(calibrated)    # (batch_tr_size,)
                batch_values += squashed

            # Average over val batches
            batch_values /= max(n_val_batches, 1)

            tr_end = tr_start + batch_tr_size
            values[tr_start:tr_end] = batch_values
            counts[tr_start:tr_end] += 1
            tr_start = tr_end

        # Restore original training order after stratified permutation
        if inv_perm is not None:
            values = values[inv_perm]

        if self.debug:
            print(f"[SAVA] train_data_values completed in {time.perf_counter() - t0:.2f}s")
            print(f"[SAVA] values: min={values.min():.4g}, max={values.max():.4g}, mean={values.mean():.4g}")
...
class SavaEvaluator(DataEvaluator, ModelLessMixin):
    ...
    This is the full SAVA algorithm. For the simpler batch-wise LAVA baseline,
    use BatchwiseLavaEvaluator.
    ...
    def train_data_values(self, *args, **kwargs):
        ...
        for i, (x_tr, y_tr) in enumerate(
            tqdm(train_batches, desc="HOT inner OT (train batches)")
        ):
