...
class DataFetcher:
    ...
    def split_dataset_by_count(
        ...
        if hasattr(self, "dataset") and self.dataset.presplit:
            # Special handling: sample train/valid from the training pool and test from the test pool.
            ...
            # Draw disjoint train/valid subsets from training pool
            trval_perm = self.random_state.permutation(train_pool_n)
            tr_idx = trval_perm[:train_count]
            val_idx = trval_perm[train_count : train_count + valid_count]

            # Draw test subset from test pool
            test_perm = self.random_state.permutation(test_pool_n)
            tst_idx = test_perm[:test_count]

            # Assign sampled splits
            ...
            # Record indices relative to their respective pools
            self.train_indices = tr_idx
            self.valid_indices = val_idx
            self.test_indices = tst_idx
